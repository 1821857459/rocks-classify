...
st.set_page_config(page_title="岩石分类与俯冲事件识别", layout="wide")
st.title("🌋 岩石分类预测 + 初始俯冲事件判定")

# 上传训练数据
train_file = st.file_uploader("📂 上传训练数据（FAB-Boninite-HMA-IAT-CA.xlsx）", type=["xlsx"])
predict_file = st.file_uploader("📂 上传预测数据（应用.xlsx）", type=["xlsx"])
...
if train_file and predict_file:
    ...
    img_path = "MgO-SiO2.jpg"  # 放置在与脚本同目录
    try:
